...
// mactop is a terminal-based Apple Silicon system monitor.
package app
...
func setupUI() {
	...
	modelText.Title = messages.Text("TUI_AppleSilicon")
	helpText.Title = messages.Text("TUI_HelpMenu")
	infoParagraph.Text = messages.Text("TUI_Loading")
	fanStatusPanel.Title = messages.Text("TUI_Fans")
	fanStatusPanel.BorderRounded = true
	fanTempPanel.Title = messages.Text("TUI_Temperatures")
	fanTempPanel.BorderRounded = true
	fanControlPanel.Title = ""
	fanControlPanel.BorderRounded = true
	modelName := appleSiliconModel.Name
	if modelName == "" {
		modelName = messages.Text("TUI_UnknownModel")
	}
	...
	processList.Title = messages.Text("TUI_ProcessList")
	...
	cpuGauge.Title = messages.Text("TUI_Loading")
	gpuGauge.Title = messages.Text("TUI_GPUUsage")
	memoryGauge.Title = messages.Text("TUI_MemoryUsage")
	aneGauge.Title = messages.Text("TUI_ANEUsage")

	PowerChart, NetworkInfo = w.NewParagraph(), w.NewParagraph()
	PowerChart.Title, NetworkInfo.Title = messages.Text("TUI_PowerUsage"), messages.Text("TUI_NetworkDisk")

	tbInfoParagraph = w.NewParagraph()
	tbInfoParagraph.Title = messages.Text("TUI_ThunderboltRDMA")
	tbInfoParagraph.Text = messages.Text("TUI_LoadingTB")
	go func() {
	...
	mainBlock.TitleBottomLeft = fmt.Sprintf(messages.Text("TUI_LayoutInfo"), currentLayoutNum, totalLayouts, currentColorName)
	mainBlock.TitleBottom = messages.Text("TUI_InfoLayoutColorExit")
	...
	gpuSparklineGroup.Title = messages.Text("TUI_GPUUsageHistory")
	...
	tbNetSparklineGroup.Title = messages.Text("TUI_TBNet")

	// StepChart widgets for History layout
	gpuHistoryChart = w.NewStepChart()
	gpuHistoryChart.Title = messages.Text("TUI_GPUUsageHistory")
	...
	powerHistoryChart.Title = messages.Text("TUI_PowerHistory")
	...
	memoryHistoryChart.Title = messages.Text("TUI_MemorySwapHistory")
	...
	cpuHistoryChart.Title = messages.Text("TUI_CPUUsageHistory")
	...
	memBWHistoryChart.Title = messages.Text("TUI_MemoryBandwidthHistory")
	...
	aneHistoryChart.Title = messages.Text("TUI_ANEUsageHistory")
	...
	bandwidthHistoryChart.Title = messages.Text("TUI_DRAMBandwidthHistory")
	...
	socPowerHistoryChart.Title = messages.Text("TUI_SoCPowerHistory")
	...
	ssdReadHistoryChart.Title = messages.Text("TUI_SSDReadHistory")
	...
	coreTitle := fmt.Sprintf(messages.Text("TUI_Cores"), totalCPUCores)
	if coreSummary != "" {
	...
	confirmModal = w.NewModal(messages.Text("TUI_ConfirmKillBody"))
	confirmModal.Title = messages.Text("TUI_ConfirmKill")
	...
	_ = confirmModal.AddButton(messages.Text("TUI_ConfirmYes"), func() {
		// Callback logic will be handled elsewhere or reused
	})
	_ = confirmModal.AddButton(messages.Text("TUI_ConfirmNo"), func() {
		// Callback logic
	})
}
...
func updateMainTitleWithHardware() {
	...
	model := info.Name
	if model == "" {
		model = messages.Text("TUI_UnknownModel")
	}
...
func updateModelText() {
	...
	cBase := messages.Text("TUI_Cores")
	cE := messages.Text("TUI_ECores")
	cP := messages.Text("TUI_PCores")
	cS := messages.Text("TUI_SCores")
	...
	modelText.Text = fmt.Sprintf("%s\n%s\n%s",
		modelName,
		coreLines,
		fmt.Sprintf(messages.Text("TUI_GPUCores"), gpuCoreCountStr),
	)
}
...
func updateHelpText() {
	...
	if maxOffset > 0 {
		if helpScrollOffset < maxOffset {
			fmt.Fprintf(&finalBuilder, "\n[%s (j/↓)](fg:%s)", messages.Text("Info_ScrollDown"), tc)
		} else {
			finalBuilder.WriteString("\n") // Spacer
		}
	}
...
func runAlternateMode() bool {
	if fanHelperInstall {
		if err := installFanHelper(); err != nil {
			fmt.Fprintf(os.Stderr, "Fan helper installation failed: %v\n", err)
			os.Exit(1)
		}
		return true
	}
	if fanHelper {
		if err := runFanHelper(); err != nil {
			fmt.Fprintf(os.Stderr, "Fan helper failed: %v\n", err)
			os.Exit(1)
		}
		return true
	}
	if fanReset {
		if err := runFanReset(); err != nil {
			fmt.Fprintf(os.Stderr, "Fan reset failed: %v\n", err)
			os.Exit(1)
		}
		fmt.Println("All fans use automatic control.")
		return true
	}
	if fanPolicy || fanPolicyDryRun {
		if err := runFanPolicy(fanPolicyDryRun); err != nil {
			fmt.Fprintf(os.Stderr, "Fan policy failed: %v\n", err)
			os.Exit(1)
		}
		return true
	}
...
func renderLoadingScreen() {
	...
	loadingBlock.Title = messages.Text("TUI_MactopTitle")
...
func Run() {
	...
	if fanControl && fanControlHasRoot() {
		fanLock, err := acquireFanControlLock(fanControlLockPath)
		if err != nil {
			stderrLogger.Fatalf("failed to start fan control: %v", err)
		}
		defer fanLock.Close()
	}
...
func updateTotalPowerChart(watts float64) {
	...
	sparkline.Data = powerValues
	sparkline.MaxVal = 8
	sparklineGroup.Title = fmt.Sprintf(messages.Text("Metrics_PowerSparklineGroup"), watts, maxPowerSeen)
	thermalStr, _ := getThermalStateString()
	sparkline.Title = fmt.Sprintf(messages.Text("Metrics_PowerSparklineTitle"), avgWatts, thermalStr)
	...
	if powerHistoryChart != nil {
		...
		powerHistoryChart.Title = fmt.Sprintf(messages.Text("Metrics_PowerHistoryDetail"), avgWatts, maxPowerSeen)
	}
}
	...
	if cpuHistoryChart != nil {
		...
		if visibleWidth > 0 {
			...
			if currentConfig.DefaultLayout == LayoutHistorySoC {
				...
				cpuHistoryChart.Title = fmt.Sprintf(messages.Text("Metrics_CPUHistoryPeak"), totalUsage, currentPeak)
				...
				cpuHistoryChart.Title = fmt.Sprintf(messages.Text("Metrics_CPUHistoryDetail"), totalUsage)
				cpuHistoryChart.DataLabels = []string{fmt.Sprintf("%.0f%%", totalUsage)}
			}
...
func updateMemoryHistory(memoryMetrics MemoryMetrics) {
	...
	if memoryHistoryChart != nil {
		...
		memoryHistoryChart.Title = fmt.Sprintf(messages.Text("Metrics_MemoryHistoryDetail"), usedGB, totalGB, swapGB)
	}
...
func renderANEHistoryChart(anePct, aneWatts, aneBW float64, bwMode bool) {
	...
	if currentConfig.DefaultLayout == LayoutHistorySoC {
		...
		if bwMode {
			...
			aneHistoryChart.Title = fmt.Sprintf(messages.Text("Metrics_ANEHistoryPeakBW"), anePct, currentPeak, aneBW)
		} else {
			aneHistoryChart.Title = fmt.Sprintf(messages.Text("Metrics_ANEHistoryPeak"), anePct, currentPeak, aneWatts)
		}
	} else {
		aneHistoryChart.LineColors = []ui.Color{historyLineColor(func(t *CustomThemeConfig) string { return t.ANE }, ui.ColorMagenta)}
		if bwMode {
			aneHistoryChart.Title = fmt.Sprintf(messages.Text("Metrics_ANEHistoryDetailBW"), anePct, aneBW)
		} else {
			aneHistoryChart.Title = fmt.Sprintf(messages.Text("Metrics_ANEHistoryDetail"), anePct, aneWatts)
		}
	}
...
func renderBandwidthHistoryChart(readGBs, writeGBs, aneReadGBs, aneWriteGBs float64) {
	...
	}
	{
		...
		if currentConfig.DefaultLayout == LayoutHistorySoC {
			...
			bandwidthHistoryChart.Title = fmt.Sprintf(messages.Text("Metrics_BandwidthHistoryPeak"), readGBs, writeGBs, aneReadGBs, aneWriteGBs, currentPeak)
			...
			total := readGBs + writeGBs
			bandwidthHistoryChart.Title = fmt.Sprintf(messages.Text("Metrics_BandwidthHistoryDetail"), readGBs, writeGBs, total) +
...
func updateSoCPowerHistory(cpuMetrics CPUMetrics) {
	...
	if socPowerHistoryChart != nil {
		...
		if aneDead {
			socPowerHistoryChart.Title = fmt.Sprintf(messages.Text("Metrics_SoCPowerDetailNoANE"),
				totalPower, cpuMetrics.CPUW, cpuMetrics.GPUW+cpuMetrics.GPUSRAMW, cpuMetrics.DRAMW)
		} else {
			socPowerHistoryChart.Title = fmt.Sprintf(messages.Text("Metrics_SoCPowerDetail"),
...
func updateMemBandwidthHistory() {
	...
	if memBWHistoryChart != nil {
		...
		memBWHistoryChart.Title = fmt.Sprintf(messages.Text("Metrics_MemBWHistoryDetail"), combinedBW)
	}
}
...
func updateCPUGaugeTitles(totalUsage float64, cpuMetrics CPUMetrics) {
	...
	if isCompactLayout() {
		cpuGauge.Title = fmt.Sprintf(messages.Text("Metrics_CPUGaugeCompact"), totalUsage, formatTemp(cpuMetrics.CPUTemp))
	} else {
		cpuGauge.Title = fmt.Sprintf(messages.Text("Metrics_CPUGauge"),
			totalCPUCores,
			coreSummary,
			totalUsage,
			cpuFreqStr,
	...
	cpuCoreWidget.Title = fmt.Sprintf(messages.Text("Metrics_CPUGauge"),
		totalCPUCores,
		coreSummary,
		totalUsage,
		cpuFreqStr,
	...
	if isCompactLayout() {
		if bwMode {
			aneGauge.Title = fmt.Sprintf(messages.Text("Metrics_ANEGaugeBWCompact"), cpuMetrics.ANEBW)
		} else {
			aneGauge.Title = fmt.Sprintf(messages.Text("Metrics_ANEGaugeCompact"), cpuMetrics.ANEW)
		}
	} else {
		if bwMode {
			aneGauge.Title = fmt.Sprintf(messages.Text("Metrics_ANEGaugeBW"), aneUtil, cpuMetrics.ANEBW)
		} else {
			aneGauge.Title = fmt.Sprintf(messages.Text("Metrics_ANEGauge"), aneUtil, cpuMetrics.ANEW)
		}
	}
...
func updatePowerChartText(cpuMetrics CPUMetrics, thermalStr string) {
	PowerChart.Title = messages.Text("TUI_PowerUsage")
	...
	if isCompactLayout() {
		...
		PowerChart.Title = messages.Text("Metrics_PowerChartTitleCompact")
		PowerChart.Text = fmt.Sprintf(messages.Text("Metrics_PowerChartTextCompact"),
			cpuMetrics.CPUW,
		...
		PowerChart.Text = fmt.Sprintf(messages.Text("Metrics_PowerChartText"),
			cpuMetrics.CPUW,
			cpuMetrics.GPUW+cpuMetrics.GPUSRAMW,
			aneStr,
			cpuMetrics.DRAMW,
...
func updateMemoryGaugeTitle(memoryMetrics MemoryMetrics) {
	if isCompactLayout() {
		memoryGauge.Title = fmt.Sprintf(messages.Text("Metrics_MemGaugeCompact"), float64(memoryMetrics.Used)/1024/1024/1024, float64(memoryMetrics.Total)/1024/1024/1024, lastCPUMetrics.DRAMBWCombined)
	} else {
		memoryGauge.Title = fmt.Sprintf(messages.Text("Metrics_MemGauge"), float64(memoryMetrics.Used)/1024/1024/1024, float64(memoryMetrics.Total)/1024/1024/1024, float64(memoryMetrics.SwapUsed)/1024/1024/1024, float64(memoryMetrics.SwapTotal)/1024/1024/1024, lastCPUMetrics.DRAMBWCombined)
	}
}
...
func updateGPUUI(gpuMetrics GPUMetrics) {
	if isCompactLayout() {
		if gpuMetrics.Temp > 0 {
			gpuGauge.Title = fmt.Sprintf(messages.Text("Metrics_GPUGaugeCompactTemp"), int(gpuMetrics.ActivePercent), formatTemp(float64(gpuMetrics.Temp)))
		} else {
			gpuGauge.Title = fmt.Sprintf(messages.Text("Metrics_GPUGaugeCompactFreq"), int(gpuMetrics.ActivePercent), gpuMetrics.FreqMHz)
		}
	} else {
		if gpuMetrics.Temp > 0 {
			gpuGauge.Title = fmt.Sprintf(messages.Text("Metrics_GPUGaugeTemp"), int(gpuMetrics.ActivePercent), gpuMetrics.FreqMHz, formatTemp(float64(gpuMetrics.Temp)))
		} else {
			gpuGauge.Title = fmt.Sprintf(messages.Text("Metrics_GPUGaugeFreq"), int(gpuMetrics.ActivePercent), gpuMetrics.FreqMHz)
		}
	}
	...
	if isCompactLayout() {
		gpuSparklineGroup.Title = fmt.Sprintf(messages.Text("Metrics_GPUSparklineCompact"), int(gpuMetrics.ActivePercent), avgGPU)
	} else {
		gpuSparklineGroup.Title = fmt.Sprintf(messages.Text("Metrics_GPUSparkline"), int(gpuMetrics.ActivePercent), avgGPU)
	}
...
func renderGPUHistoryChart(gpuMetrics GPUMetrics, avgGPU, effectiveNow float64) {
	...
	if currentConfig.DefaultLayout == LayoutHistorySoC {
		...
		gpuHistoryChart.Title = fmt.Sprintf(messages.Text("Metrics_GPUHistoryEff"),
		...
		gpuHistoryChart.LineColors = []ui.Color{historyLineColor(func(t *CustomThemeConfig) string { return t.GPU }, ui.ColorGreen)}
		gpuHistoryChart.Title = fmt.Sprintf(messages.Text("Metrics_GPUHistoryChart"), avgGPU)
...
func updateNetDiskUI(netdiskMetrics NetDiskMetrics) {
	...
	if ssdReadHistoryChart != nil {
		...
		ssdReadHistoryChart.Title = fmt.Sprintf(messages.Text("Metrics_SSDReadDetail"), readGBs, peak)
	}
	...
	if linkInfo != "" {
		fmt.Fprintf(&sb, messages.Text("Metrics_NetLink")+"\n", linkInfo, netOut, netIn)
	} else {
		fmt.Fprintf(&sb, messages.Text("Metrics_Net")+"\n", netOut, netIn)
	}
	...
	fmt.Fprintf(&sb, messages.Text("Metrics_IO")+"\n", diskRead, diskWrite)
	...
	for i, v := range volumes {
		...
		fmt.Fprintf(&sb, messages.Text("Metrics_DiskFree")+"\n", v.Name, used, total, avail)
	}
...
func updateTBNetUI(tbStats []ThunderboltNetStats) {
	...
	rdmaStatus := CheckRDMAAvailable()
	rdmaLabel := fmt.Sprintf("%s: %s", messages.Text("Info_RDMA"), messages.Text("Info_Disabled"))
	if rdmaStatus.Available {
		rdmaLabel = fmt.Sprintf("%s: %s", messages.Text("Info_RDMA"), messages.Text("Info_Enabled"))
	}
	...
	tbInfoMutex.Unlock()
	if tbDeviceInfo == "" {
		tbDeviceInfo = messages.Text("TUI_Loading")
	}

	// Show RDMA status and bandwidth in text, above device list
	tbInfoParagraph.Text = fmt.Sprintf("%s | %s: ↓%s/s ↑%s/s\n%s", rdmaLabel, messages.Text("Info_TBNet"), inStr, outStr, tbDeviceInfo)
	...
	if tbNetSparklineGroup != nil {
		tbNetSparklineGroup.Title = fmt.Sprintf("%s: ↓%s/s ↑%s/s", messages.Text("Info_TBNet"), inStr, outStr)
...
func parseCommandLineFlags() {
	...
	flag.BoolVar(&fanPolicy, "fan-policy", false, "Run the fixed CPU P-core fan policy (requires root; no UI)")
	flag.BoolVar(&fanPolicyDryRun, "fan-policy-dry-run", false, "Calculate one fixed fan policy sample without SMC writes")
	flag.BoolVar(&fanReset, "fan-reset", false, "Restore all fans to automatic control and exit (requires root)")
	flag.BoolVar(&fanHelper, "fan-helper", false, "Run the privileged fan helper (internal)")
	flag.BoolVar(&fanHelperInstall, "install-fan-helper", false, "Install the privileged fan helper (internal)")
