...
func getProcessListTitle() (string, ui.Style) {
	...
	if killPending {
		return fmt.Sprintf(messages.Text("TUI_ProcessListKill"), killPID), ui.NewStyle(ui.ColorRed, CurrentBgColor, ui.ModifierBold)
	} else if searchMode || searchText != "" {
		return fmt.Sprintf(messages.Text("TUI_ProcessListSearch"), searchText), ui.NewStyle(titleColor, CurrentBgColor, ui.ModifierBold)
	} else if isFrozen {
		return messages.Text("TUI_ProcessListFrozen"), ui.NewStyle(titleColor, CurrentBgColor, ui.ModifierBold)
	} else if filterPID > 0 {
		return fmt.Sprintf(messages.Text("TUI_ProcessListPID"), filterPID), ui.NewStyle(titleColor, CurrentBgColor, ui.ModifierBold)
	}
	return messages.Text("TUI_ProcessListFull"), ui.NewStyle(titleColor, CurrentBgColor)
}
