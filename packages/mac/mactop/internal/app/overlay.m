...
- (void)drawRect:(NSRect)dirtyRect {
  ...
  if (modelName.length == 0)
    modelName = messageText(@"Menu_AppleSilicon");
  ...
  if (m.gpu_core_count > 0) {
    [coreSummary appendFormat:@" • %@", [NSString stringWithFormat:messageText(@"Overlay_GPUCores"), m.gpu_core_count]];
  }
  ...
  if (g_showSettings) {
    ...
    // --- Collapsed Mode ---
    {
      NSString *header = messageText(@"Overlay_CollapsedMode");
    ...
    // --- Expanded Mode ---
    {
      NSString *header = messageText(@"Overlay_ExpandedMode");
      NSDictionary *hdrAttrs = @{
        NSFontAttributeName : sectionTitleFont,
        NSForegroundColorAttributeName : overlayNeonGreen()
      };
    ...
    // Done button
    {
      ...
      NSString *doneText = messageText(@"Overlay_Done");
  ...
  void (^drawSectionFPS)(void) = ^{
    ...
    if (g_fpsStreamFailed) {
      NSString *warn = messageText(@"Overlay_RequiresScreenRecordingPermission");
  ...
  void (^drawSectionFrame)(void) = ^{
    ...
    if (g_fpsStreamFailed) {
      NSString *warn = messageText(@"Overlay_RequiresPermission");
      NSDictionary *warnAttrs = @{
        NSFontAttributeName : [NSFont systemFontOfSize:13 weight:NSFontWeightMedium],
        NSForegroundColorAttributeName : overlayAccentRed()
      };
  ...
  void (^drawSectionPower)(void) = ^{
    ...
    drawMetricKV([NSString stringWithFormat:@"  %@", messageText(@"Menu_CPU")], [NSString stringWithFormat:@"%.1fW", m.cpu_watts], overlayDimText());
    drawMetricKV([NSString stringWithFormat:@"  %@", messageText(@"Menu_GPU")], [NSString stringWithFormat:@"%.1fW", m.gpu_watts], overlayDimText());
    drawMetricKV([NSString stringWithFormat:@"  %@", messageText(@"Overlay_ANE")], [NSString stringWithFormat:@"%.1fW", m.ane_watts], overlayDimText());
    drawMetricKV([NSString stringWithFormat:@"  %@", messageText(@"Overlay_DRAM")], [NSString stringWithFormat:@"%.1fW", m.dram_watts], overlayDimText());
  };
  ...
  void (^drawSectionGPUFreq)(void) = ^{
    NSString *freqStr;
    if (m.tflops_fp32 > 0) {
      freqStr = [NSString stringWithFormat:messageText(@"Overlay_GPUFreqTFLOPs"),
                                          m.gpu_freq_mhz, m.tflops_fp32];
    } else {
      freqStr = [NSString stringWithFormat:messageText(@"Overlay_GPUFreqMHz"),
                                          m.gpu_freq_mhz];
    }
  ...
  void (^drawSectionTemps)(void) = ^{
    NSString *tempStr;
    if (m.gpu_temp > 0) {
      tempStr = [NSString stringWithFormat:messageText(@"Overlay_TempsDual"),
                                          m.cpu_temp, m.gpu_temp];
    } else {
      tempStr = [NSString stringWithFormat:messageText(@"Overlay_TempsSingle"),
                                          m.cpu_temp];
    }
  ...
  void (^drawSectionThermal)(void) = ^{
    ...
    if (thermalStr.length == 0)
      thermalStr = messageText(@"Metrics_ThermalUnknown");
    NSColor *thermalColor = overlayAccentGreen();
    // Use the numeric thermal level when selecting a color.
  ...
  if (g_opacityFlashCountdown > 0) {
    ...
    NSString *opacityStr =
        [NSString stringWithFormat:messageText(@"Overlay_OpacityHint"), opacityPct];
