...
var (
	...
	fanPolicy        bool    // Run the fixed CPU P-core fan policy (requires root)
	fanPolicyDryRun  bool    // Calculate one fan policy sample without SMC writes
	fanReset         bool    // Restore all fans to automatic control and exit
	fanHelper        bool    // Hidden: run the privileged fixed-policy service
	fanHelperInstall bool    // Hidden: install the privileged fixed-policy service
