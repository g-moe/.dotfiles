...
func runHeadless(count int) {
	if err := initSocMetrics(); err != nil {
		fmt.Fprintf(os.Stderr, messages.Text("Headless_ErrorInitMetrics")+"\n", err)
		os.Exit(1)
	}
	...
	default:
		fmt.Fprintf(os.Stderr, messages.Text("Headless_ErrorUnknownFormat")+"\n", format)
		format = "json"
	}
	...
	if err := processHeadlessSample(format, tbInfo, cachedHeadlessSysInfo); err != nil {
		fmt.Fprintf(os.Stderr, messages.Text("Headless_ErrorFormattingOutput")+"\n", err)
	}
	samplesCollected++
