...
// SMCGetFloatValueChecked keeps the read status separate from a valid zero.
// Several SMC mode keys use zero for automatic control, so callers must not
// treat the legacy getter's zero-on-error behavior as a successful readback.
kern_return_t SMCGetFloatValueChecked(io_connect_t conn, const char *key,
                                      double *outputValue) {
  if (outputValue == NULL) {
  ...
  *outputValue = 0.0;
  ...
  if (SMCIsType(val.keyInfo.dataType, "flt ") &&
    ...
    *outputValue = (double)f;
    return kIOReturnSuccess;
  }
  if (SMCIsType(val.keyInfo.dataType, "ui8 ") &&
      val.keyInfo.dataSize >= 1) {
    *outputValue = (double)(unsigned char)val.bytes[0];
    return kIOReturnSuccess;
  }
...
double SMCGetFloatValue(io_connect_t conn, const char *key) {
  double value = 0.0;
  if (SMCGetFloatValueChecked(conn, key, &value) != kIOReturnSuccess) {
    return 0.0;
  }
  return value;
}
