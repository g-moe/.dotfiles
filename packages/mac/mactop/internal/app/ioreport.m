...
static int readFanInfoFromConnection(io_connect_t connection,
  ...
  // Read number of fans
  SMCKeyData_t val;
  if (SMCReadKey(connection, "FNum", &val) != kIOReturnSuccess)
    return 0;
  ...
  for (int i = 0; i < fanCount; i++) {
    ...
    fans[i].actualRPM = (int)SMCGetFloatValue(connection, key);

    // Read min RPM: F%dMn
    snprintf(key, sizeof(key), "F%dMn", i);
    fans[i].minRPM = (int)SMCGetFloatValue(connection, key);

    // Read max RPM: F%dMx
    snprintf(key, sizeof(key), "F%dMx", i);
    fans[i].maxRPM = (int)SMCGetFloatValue(connection, key);
...
static int readFanInfo(fan_info_t *fans, int maxFans) {
  return readFanInfoFromConnection(g_smcConn, fans, maxFans);
}

// Helper startup only needs fan bounds. Open a short-lived SMC connection and
// avoid the timed IOReport sample used for power and temperature metrics.
PowerMetrics readFanMetrics(void) {
  ...
  metrics.fanCount = readFanInfoFromConnection(connection, metrics.fans, 8);
  SMCClose(connection);
  return metrics;
}
...
int setFanTarget(int fanIndex, int rpm) {
  if (!g_smcConn || fanIndex < 0 || fanIndex > 7 || rpm <= 0)
    return -1;
