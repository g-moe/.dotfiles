...
PowerMetrics readFanMetrics(void);
...
// readFanMetrics reads only the SMC fan keys. Helper startup needs fan limits,
// not a timed IOReport power sample, so this avoids an unrelated 500 ms wait.
func readFanMetrics() ([]FanInfo, error) {
	metrics := C.readFanMetrics()
	if metrics.fanCount < 0 {
		return nil, fmt.Errorf("failed to read fan metrics")
	}
	fans := make([]FanInfo, int(metrics.fanCount))
	for i := 0; i < int(metrics.fanCount) && i < 8; i++ {
		fan := metrics.fans[i]
		fans[i] = FanInfo{
			ID:        int(fan.id),
			Name:      C.GoString(&fan.name[0]),
			ActualRPM: int(fan.actualRPM),
			MinRPM:    int(fan.minRPM),
			MaxRPM:    int(fan.maxRPM),
			TargetRPM: int(fan.targetRPM),
			Mode:      int(fan.mode),
		}
	}
	return fans, nil
...
func SetFanTarget(fanIndex, rpm int) error {
	...
	if rpm <= 0 {
		return fmt.Errorf("invalid fan target %d RPM", rpm)
	}
