...
var (
	...
	fanSettingsRequestMu  sync.Mutex
)
...
func copyStringToCBuf(dst []C.char, value string) {
	b := []byte(value)
	if len(b) >= len(dst) {
		b = b[:len(dst)-1]
	}
...
func menuBarLayoutToInt(layout string) C.int {
	if layout == "two_row" {
		return 1
	}
	return 0
...
func applyMenuBarConfig() {
	...
	// ANE remains available in the full monitor, but it is intentionally not a
	// menu-bar status item. Ignore older configs that enabled it.
	ccfg.show_ane = 0
	ccfg.show_memory = boolToInt(mbCfg.ShowMemory, 0)
	ccfg.show_fan = boolToInt(mbCfg.ShowFan, 0)
	...
	copyStringToCBuf(ccfg.cpu_color[:], mbCfg.CPUColor)
	copyStringToCBuf(ccfg.gpu_color[:], mbCfg.GPUColor)
	copyStringToCBuf(ccfg.ane_color[:], mbCfg.ANEColor)
	copyStringToCBuf(ccfg.mem_color[:], mbCfg.MemColor)
	copyStringToCBuf(ccfg.label_color[:], mbCfg.LabelColor)
	copyStringToCBuf(ccfg.bar_order[:], mbCfg.BarOrder)
	copyStringToCBuf(ccfg.cpu_label[:], mbCfg.CPULabel)
	copyStringToCBuf(ccfg.gpu_label[:], mbCfg.GPULabel)
	copyStringToCBuf(ccfg.ane_label[:], mbCfg.ANELabel)
	copyStringToCBuf(ccfg.memory_label[:], mbCfg.MemoryLabel)
	ccfg.cpu_two_row = menuBarLayoutToInt(mbCfg.CPULayout)
	ccfg.gpu_two_row = menuBarLayoutToInt(mbCfg.GPULayout)
	ccfg.ane_two_row = menuBarLayoutToInt(mbCfg.ANELayout)
	ccfg.memory_two_row = menuBarLayoutToInt(mbCfg.MemoryLayout)
	ccfg.fan_two_row = menuBarLayoutToInt(mbCfg.FanLayout)
	ccfg.bar_border_width = C.int(mbCfg.BarBorderWidth)
	copyStringToCBuf(ccfg.bar_border_color[:], mbCfg.BarBorderColor)
	ccfg.bar_height = C.int(mbCfg.BarHeight)
...
// GoMessageText provides U.S. English message lookup to Objective-C.
//
//export GoMessageText
func GoMessageText(id *C.char) *C.char {
	goID := C.GoString(id)
	message := messages.Text(goID)
	// Return malloc'd string (caller must free)
	return C.CString(message)
}

const (
	fanSettingsUnavailable = iota
	fanSettingsOff
	fanSettingsStarting
	fanSettingsActive
	fanSettingsStopping
...
func fanSettingsDisplay(status fanHelperStatus, requestErr error) (int, string, string) {
	if requestErr != nil {
		if errors.Is(requestErr, errFanHelperVersionMismatch) {
			return fanSettingsUpdateRequired, messages.Text("Menu_FanCurveDetail"), messages.Text("Menu_FanHelperUpdateRequired")
		}
		if errors.Is(requestErr, os.ErrNotExist) {
			return fanSettingsUnavailable, messages.Text("Menu_FanCurveDetail"), messages.Text("Menu_FanHelperNotInstalled")
		}
		return fanSettingsHelperError, messages.Text("Menu_FanCurveDetail"), fmt.Sprintf(messages.Text("Menu_FanHelperUnhealthy"), requestErr)
	}
	switch status.State {
	case "off":
	...
	case "stopping":
		return fanSettingsStopping, messages.Text("Menu_FanPolicyStoppingDetail"), ""
	case "active":
		ids := make([]int, 0, len(status.Targets))
		for id := range status.Targets {
			ids = append(ids, id)
		}
		sort.Ints(ids)
		parts := make([]string, 0, len(ids))
		for _, id := range ids {
			parts = append(parts, fmt.Sprintf(messages.Text("Menu_FanTargetDetail"), id, status.Targets[id]))
		}
		if status.Mode == fanModeConstant {
			return fanSettingsActive, strings.Join(parts, " • "), status.Message
		}
		return fanSettingsActive, fmt.Sprintf(messages.Text("Menu_FanPolicyActiveDetail"), status.Temperature, strings.Join(parts, " • ")), status.Message
	case "error":
		return fanSettingsError, messages.Text("Menu_FanAutomaticFallback"), status.Message
	default:
		return fanSettingsError, "", messages.Text("Menu_FanHelperUnknownState")
	}
}

const (
	fanSettingsResponsePoll = iota
	fanSettingsResponseMutation
)

func postFanSettingsStatus(status fanHelperStatus, requestErr error, generation uint64, responseSource int) {
	state, detail, message := fanSettingsDisplay(status, requestErr)
	...
	if status.Mode == fanModeConstant {
		mode = 1
	} else if status.Mode == fanModeCurve {
		mode = 2
	}
	C.updateMenuBarFanPolicyStatus(
		C.int(state), C.int(mode), C.int(status.ConstantRPM),
		C.double(status.StartCelsius), C.double(status.MaximumCelsius),
		C.int(status.MinimumRPM), C.int(status.MaximumRPM), C.int(map[string]int{"available": 1, "unavailable": 2}[status.RPMRangeState]),
		C.ulonglong(generation), C.int(responseSource),
		cDetail, cMessage,
	)
}

func runFanSettingsRequest(request fanHelperRequest, coalesce bool, generation uint64, responseSource int) {
	if coalesce && !fanSettingsRequestMu.TryLock() {
		return
	}
	if !coalesce {
		fanSettingsRequestMu.Lock()
	}
	defer fanSettingsRequestMu.Unlock()
	status, err := requestFanHelperRequest(request)
	postFanSettingsStatus(status, err, generation, responseSource)
}

// GoRequestFanPolicyStatus is called when the Settings window opens.
//
//export GoRequestFanPolicyStatus
func GoRequestFanPolicyStatus(generation C.ulonglong) {
	go runFanSettingsRequest(
		fanHelperRequest{Command: "status"}, true, uint64(generation), fanSettingsResponsePoll,
	)
}

// GoConfigureFanControl applies one validated helper-owned control mode.
//
//export GoConfigureFanControl
func GoConfigureFanControl(mode, constantRPM C.int, startCelsius, maximumCelsius C.double, generation C.ulonglong) {
	modeName := fanModeDefault
	if mode == 1 {
		modeName = fanModeConstant
	} else if mode == 2 {
		modeName = fanModeCurve
	}
	request := fanHelperRequest{
		Command: "configure", Mode: modeName,
	}
	if modeName == fanModeConstant {
		request.ConstantRPM = int(constantRPM)
	} else if modeName == fanModeCurve {
		request.StartCelsius = float64(startCelsius)
		request.MaximumCelsius = float64(maximumCelsius)
	}
	go runFanSettingsRequest(request, false, uint64(generation), fanSettingsResponseMutation)
}
...
func installFanHelperWithAuthorization() (fanHelperStatus, error) {
	...
	if digest == "" {
		return fanHelperStatus{}, errors.New("could not identify the mactop build")
	}
	command := authorizedFanHelperInstallCommand(executable, digest)
	...
	if output, err := exec.Command("/usr/bin/osascript", "-e", script).CombinedOutput(); err != nil {
		return fanHelperStatus{}, fmt.Errorf("authorization failed: %s", strings.TrimSpace(string(output)))
	}
	return waitForFanHelperReadinessAt(fanHelperSocketPath, 5*time.Second)
}

func waitForFanHelperReadinessAt(socketPath string, timeout time.Duration) (fanHelperStatus, error) {
	...
	for {
		...
		requestTimeout := min(remaining, fanHelperRequestTimeout)
		status, requestErr := requestFanHelperRequestAtWithTimeout(
			socketPath, fanHelperRequest{Command: "status"}, requestTimeout,
		)
		if requestErr == nil {
			return status, nil
		}
	...
	return fanHelperStatus{}, errors.New("fan helper did not start")
}

// authorizedFanHelperInstallCommand copies the candidate to a root-created
// temporary file, then verifies the bytes that will execute. The digest was
// calculated before the authorization dialog, so replacing the user-owned
// source while that dialog is open cannot substitute root code.
func authorizedFanHelperInstallCommand(executable, expectedDigest string) string {
	return "staged=$(/usr/bin/mktemp /var/tmp/mactop-fan-helper-install.XXXXXX) || exit 1; " +
		"trap '/bin/rm -f \"$staged\"' EXIT; " +
		"/usr/bin/install -o root -g wheel -m 0755 " + shellQuote(executable) + " \"$staged\" || exit 1; " +
		"actual=$(/usr/bin/shasum -a 256 \"$staged\" | /usr/bin/awk '{print $1}'); " +
		"[ \"$actual\" = " + shellQuote(expectedDigest) + " ] || exit 1; " +
		"\"$staged\" --install-fan-helper"
}

// GoInstallFanHelper installs a root-owned copy after native authorization.
//
//export GoInstallFanHelper
func GoInstallFanHelper(generation C.ulonglong) {
	go func() {
		fanSettingsRequestMu.Lock()
		defer fanSettingsRequestMu.Unlock()
		status, err := installFanHelperWithAuthorization()
		if err != nil {
			postFanSettingsStatus(fanHelperStatus{}, err, uint64(generation), fanSettingsResponseMutation)
			return
		}
		postFanSettingsStatus(status, nil, uint64(generation), fanSettingsResponseMutation)
	}()
}
