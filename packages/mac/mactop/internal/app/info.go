...
func buildInfoLines(themeColor string) []string {
	...
	rdmaLabel := messages.Text("Info_Disabled")
	if rdmaStatus.Available {
		rdmaLabel = messages.Text("Info_Enabled")
	}

	infoLines := []string{
		fmt.Sprintf("[%s@%s](fg:%s,mod:bold)", cachedCurrentUser, cachedHostname, themeColor),
		"-------------------------",
		formatLine(messages.Text("Info_OS"), fmt.Sprintf(messages.Text("Info_OSValue"), cachedOSVersion)),
		formatLine(messages.Text("Info_Host"), cachedModelName),
		formatLine(messages.Text("Info_Kernel"), cachedKernelVersion),
		formatLine(messages.Text("Info_Uptime"), uptimeStr),
		formatLine(messages.Text("Info_Shell"), cachedShell),
		formatLine(messages.Text("Info_CPU"), cachedModelName),
		formatLine(messages.Text("Info_GPU"), func() string {
			if appleSiliconModel.GPUCoreCount <= 0 {
				return messages.Text("Info_NotAvailable")
			}
			return fmt.Sprintf(messages.Text("TUI_GPUCores"), strconv.Itoa(appleSiliconModel.GPUCoreCount))
		}()),
		formatLine(messages.Text("Info_TFLOPs"), func() string {
			...
			if maxFreq > 0 && gpuCores > 0 {
				...
				return fmt.Sprintf(messages.Text("Info_TFLOPsValue"), fp32TFLOPs, fp16TFLOPs)
			}
			return messages.Text("Info_NotAvailable")
		}()),
		formatLine(messages.Text("Info_Memory"), fmt.Sprintf("%.2f GB / %.2f GB", usedMem, totalMem)),
		formatLine(messages.Text("Info_Swap"), fmt.Sprintf("%.2f GB / %.2f GB", swapUsed, swapTotal)),
		"",
		formatLine(messages.Text("Info_CPUUsage"), fmt.Sprintf("%.2f%%", float64(cpuGauge.Percent))),
		formatLine(messages.Text("Info_GPUUsage"), fmt.Sprintf("%d%%", int(lastGPUMetrics.ActivePercent))),
		formatLine(messages.Text("Info_ANEUsage"), fmt.Sprintf("%d%%", int(aneUtilizationPercent(lastCPUMetrics)))),
		formatLine(messages.Text("Info_Power"), fmt.Sprintf(messages.Text("Info_PowerValue"), lastCPUMetrics.PackageW, avgWatts)),
		formatLine(messages.Text("Info_Thermals"), thermalStr),
		formatLine(messages.Text("Info_Network"), fmt.Sprintf(messages.Text("Info_NetworkValue"), formatBytes(lastNetDiskMetrics.OutBytesPerSec, networkUnit), formatBytes(lastNetDiskMetrics.InBytesPerSec, networkUnit))),
		formatLine(messages.Text("Info_Disk"), fmt.Sprintf(messages.Text("Info_DiskValue"), formatBytes(lastNetDiskMetrics.ReadKBytesPerSec*1024, diskUnit), formatBytes(lastNetDiskMetrics.WriteKBytesPerSec*1024, diskUnit))),
		formatLine(messages.Text("Info_DRAMBW"), fmt.Sprintf(messages.Text("Info_DRAMBWValue"), lastCPUMetrics.DRAMReadBW, lastCPUMetrics.DRAMWriteBW, lastCPUMetrics.DRAMBWCombined)),
	}

	if bat := GetBatteryInfo(); bat.Displayable() {
		infoLines = append(infoLines, formatLine(messages.Text("Info_Battery"), fmt.Sprintf(messages.Text("Info_BatteryValue"), *bat.Percent, batteryStateLabel(bat))))
	}
	...
	if len(lastCPUMetrics.Fans) > 0 {
		...
		for _, fan := range lastCPUMetrics.Fans {
			modeStr := messages.Text("Info_Auto")
			if fan.Mode == 1 {
				modeStr = messages.Text("Info_Manual")
			}
			infoLines = append(infoLines, formatLine(fan.Name, fmt.Sprintf(messages.Text("Info_FanValue"), fan.ActualRPM, modeStr, fan.MinRPM, fan.MaxRPM)))
		}
	}
	...
	infoLines = append(infoLines, formatLine(messages.Text("Info_TBNet"), fmt.Sprintf(messages.Text("Info_NetworkValue"), tbOut, tbIn)))

	infoLines = append(infoLines, formatLine(messages.Text("Info_RDMA"), rdmaLabel))
...
func buildNetworkLinkLines(formatLine func(string, string) string) []string {
	...
	for _, eth := range ethInfo {
		if eth.LinkUp {
			lines = append(lines, formatLine(messages.Text("Info_Ethernet"), fmt.Sprintf("%s (%s)", FormatLinkSpeed(eth.LinkSpeedMbps), eth.Name)))
		} else {
			lines = append(lines, formatLine(messages.Text("Info_Ethernet"), fmt.Sprintf(messages.Text("Info_Disconnected"), eth.Name)))
		}
	}
	if wifiInfo != nil {
		if wifiInfo.IsConnected {
			...
			lines = append(lines, formatLine(messages.Text("Info_WiFi"), fmt.Sprintf(messages.Text("Info_WiFiValue"), gen, wifiInfo.TxRateMbps, wifiInfo.InterfaceName)))
		} else {
			lines = append(lines, formatLine(messages.Text("Info_WiFi"), fmt.Sprintf(messages.Text("Info_Disconnected"), wifiInfo.InterfaceName)))
		}
	}
	return lines
}
...
func buildVolumeLines(formatLine func(string, string) string) []string {
	...
	if len(volumes) > 0 {
		...
		for _, v := range volumes {
			...
			lines = append(lines, formatLine(v.Name, fmt.Sprintf(messages.Text("Info_VolumeValue"), used, total, avail)))
		}
	}
	return lines
}
...
func renderInfoText(infoLines, asciiArt []string, layout infoLayout, themeColor string) string {
	...
	if infoScrollOffset > 0 {
		fmt.Fprintf(&combinedText, "%s[%s (k/↑)](fg:%s)\n", paddingStr, messages.Text("Info_ScrollUp"), themeColor)
	}
	...
	if layout.endLine < layout.totalLines {
		fmt.Fprintf(&combinedText, "%s[%s (j/↓)](fg:%s)\n", paddingStr, messages.Text("Info_ScrollDown"), themeColor)
	}
...
func fanRPMBar(fan FanInfo, themeColor string) []string {
	modeStr := messages.Text("Fan_Mode_Auto")
	modeColor := "green"
	if fan.Mode == 1 {
		modeStr = messages.Text("Fan_Mode_Manual")
		modeColor = "yellow"
	}
	...
	return []string{
		...
		fmt.Sprintf("    "+messages.Text("Fan_TargetRange"),
...
// tempCategoryKeys maps internal U.S. English category names (used for grouping
// in buildGroupedTempLines) to their message IDs. Pluralized variants
...
// tempCategoryName returns the U.S. English display name for an internal
// U.S. English category key. When plural is true and the category has a
// pluralized form (E/P/S-Cores), that is used instead.
func tempCategoryName(cat string, plural bool) string {
	if plural {
		if k, ok := tempCategoryPluralKeys[cat]; ok {
			return messages.Text(k)
		}
	}
...
func formatTempGroupLine(cat string, g *tempGroup, themeColor string) string {
	...
	displayName := tempCategoryName(cat, g.count > 1)
	if g.count == 1 {
		return fmt.Sprintf("  [%-16s](fg:%s)  [%s](fg:%s)",
			displayName, themeColor, formatTemp(avg), tempColor)
	}
	return fmt.Sprintf("  [%-16s](fg:%s)  [%s](fg:%s)  ["+messages.Text("Temp_AvgOf")+"](fg:%s)",
		displayName, themeColor, formatTemp(avg), tempColor,
...
func buildFanStatusText(themeColor string) string {
	...
	lines = append(lines, fmt.Sprintf("[%s](fg:%s,mod:bold): [%s](fg:%s)", messages.Text("Overlay_Thermal"), themeColor, thermalStr, themeColor))
	lines = append(lines, "")

	// CPU/GPU quick temps
	if lastCPUMetrics.CPUTemp > 0 {
		lines = append(lines, formatLine(messages.Text("Fan_CPUTemp"), formatTemp(lastCPUMetrics.CPUTemp)))
	}
	if lastCPUMetrics.GPUTemp > 0 {
		lines = append(lines, formatLine(messages.Text("Fan_GPUTemp"), formatTemp(lastCPUMetrics.GPUTemp)))
	}
	lines = append(lines, "")
	...
	if len(lastCPUMetrics.Fans) > 0 {
		...
		lines = append(lines, fmt.Sprintf("[%s](fg:%s)", messages.Text("Fan_NoFansDetected"), themeColor))
	}
...
func buildFanTempText(themeColor string) string {
	...
	if len(lastCPUMetrics.TempSensors) > 0 {
		...
		lines = append(lines, fmt.Sprintf("[%s](fg:%s)", messages.Text("Fan_NoTemperatureSensorsDetected"), themeColor))
	}
...
func buildFanControlText(themeColor string) string {
	if !fanControl {
		return fmt.Sprintf(messages.Text("Fan_ReadOnly"),
	...
	if !fanControlHasRoot() {
		return messages.Text("Fan_NeedsRoot")
	}
	base := fmt.Sprintf(messages.Text("Fan_ControlActive"),
	...
	if fanControlWriteFailed {
		base += messages.Text("Fan_WriteFailed")
	}
	return base
}
