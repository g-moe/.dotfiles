...
- **better-vscode: terminal - focus** — create or reuse the extension's `terminal` tab,
...
- **better-vscode: terminal - focus & maximize** — do the same, then maximize
  group 1. Both sidebars keep their visibility setting. Other editor groups
  stay open but are hidden.
...
New shells use the name `terminal`; existing shells keep their current name.
...
If the secondary sidebar is maximized, focusing the terminal restores the saved
sidebar layout first.
