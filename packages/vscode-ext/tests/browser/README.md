...
The suite has 46 cases for terminal placement, focus, lifecycle, and speed.
...
views. The command checks all four sidebar visibility combinations. A maximized
secondary sidebar must restore the saved sidebar layout when the editor gets focus.
The temporary workspace has its own Git repository.
The browser adapter in `check.js` exports `checkCheckpoint(browser, keyboard,
id, phase, proofPath)`. Call it with `before` and `after` for cases 1–46. Pass the
browser handle with `.playwright`, the keyboard handle with `.pressKey()` and
`.typeText()`, and the printed report path plus `.input`.

F17 invokes the public terminal command directly from workbench controls; other
cases use the Command Palette. F19 continues the test. The adapter checks the UI,
then types proof through the current focus. Do not focus the terminal before
typing. Test commands, bindings, and files are excluded from the VSIX.
...
overhead. Five warm samples per Search/Explorer and maximize state must
