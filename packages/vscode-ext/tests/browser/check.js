...
export async function readUI(browser) {
	return browser.playwright.evaluate(() => {
		...
		return {
			...
			primarySidebar: visible(".part.sidebar"),
			secondarySidebar: visible(".part.auxiliarybar"),
...
export async function checkCheckpoint(browser, keyboard, id, phase, proofPath) {
	...
	let restoredSidebars;

	// A maximized secondary sidebar temporarily hides the primary sidebar.
	// Compare the saved layout, which VS Code restores when an editor gets focus.
	if (phase === "before" && check.state === "sidebar-maximized") {
		await palette(
			browser,
			keyboard,
			"View: Toggle Maximized Secondary Side Bar",
		);
		await keyboard.pressKey("Return");
		restoredSidebars = await readUI(browser);
		await palette(browser, keyboard, "View: Maximize Secondary Side Bar");
		await keyboard.pressKey("Return");
	}
	...
	// Set all four visibility combinations before dispatch, outside timing.
	if (phase === "before" && check.state?.startsWith("sidebars-")) {
		const state = await readUI(browser);
		const desired = check.state.slice("sidebars-".length);

		for (const [current, wanted, title] of [
			[
			...
			if (current !== wanted) {
				await palette(browser, keyboard, title);
				await keyboard.pressKey("Return");
			}
		}

		await palette(browser, keyboard, "View: Focus Active Editor Group");
		await keyboard.pressKey("Return");
	}
	...
	if (phase === "before" && check.state?.startsWith("sidebars-")) {
		...
		if (
			ui.primarySidebar !== (desired[0] === "1") ||
			ui.secondarySidebar !== (desired[1] === "1")
		) {
			throw new Error("Wrong starting sidebar visibility");
		}
	}
	...
	if (phase === "before") {
		if (check.command) {
			...
			const expectedSidebars = restoredSidebars ?? ui;

			if (
				expectedSidebars.primarySidebar !== ready.primarySidebar ||
				expectedSidebars.secondarySidebar !== ready.secondarySidebar
			) {
				throw new Error(`Case ${id}: command changed sidebar visibility`);
			}
