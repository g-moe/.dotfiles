...
test_one_brewfile_and_environment_loading() {
  ...
  assert_file_contains "$SCRIPTS_DIR/mac/install/mac-system-settings.sh" 'load_homebrew' 'Mac system settings child must reload Homebrew'
  assert_file_contains "$brewfile" 'brew "imagemagick"' 'Mac wallpaper generation requires ImageMagick'
  assert_file_contains "$SCRIPTS_DIR/mac/install/mac-system-settings.sh" '.machine-wallpaper-white-$color_key-rgb-rotated-vignette-25.png' 'Mac wallpaper filename must change with its source, color, and effect'
  ...
  assert_file_contains "$SCRIPTS_DIR/mac/install/mac-system-settings.sh" 'source_path="$CONFIG_DIR/white.heic"' 'Mac wallpaper must use the tracked white source image'
  assert_file_contains "$SCRIPTS_DIR/mac/install/mac-system-settings.sh" '-rotate 180' 'Mac wallpaper source must be rotated 180 degrees'
  assert_file_contains "$SCRIPTS_DIR/mac/install/mac-system-settings.sh" "+level-colors '#000000',\"\$MACHINE_COLOR_HEX\"" 'Mac wallpaper light must be mapped to the selected machine color'
  assert_file_contains "$SCRIPTS_DIR/mac/install/mac-system-settings.sh" "radial-gradient:white-black +level '25%,100%'" 'Mac wallpaper must use the approved vignette strength'
  assert_file_contains "$SCRIPTS_DIR/mac/install/mac-system-settings.sh" '-compose multiply' 'Mac wallpaper must multiply the vignette mask into the image'
  ...
  assert_file_contains "$SCRIPTS_DIR/mac/install/mac-system-settings.sh" "interactive_select 'Change the desktop wallpaper?'" 'Mac wallpaper setup must ask before changing the wallpaper'
  assert_file_contains "$SCRIPTS_DIR/mac/install/mac-system-settings.sh" "'Skip (leave the current wallpaper)'" 'Mac wallpaper setup must allow leaving the wallpaper unchanged'
