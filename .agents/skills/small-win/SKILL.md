---
name: small-win
description: Use only for $small-win to find, prove, implement, and unanimously review one small codebase win or improvement.
...
2. Find and implement one small win or improvement.
...
4. Present the small fix, proof, and results to the `$agent-team` team.
5. If the team votes 3-0 to approve, explain to the user why it is a win and what it solves or improves.
6. If the team cannot reach a 3-0 approval after discussion, move on to a new small win or improvement.
...
- Use `$agent-team` for the review. The three subagents cast the votes; the current agent stays in charge.
